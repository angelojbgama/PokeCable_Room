...
O projeto tambem possui modo de batalha separado do fluxo de troca. A batalha usa o modelo canonico dos Pokemon da party e uma engine local em processo; ela nao escreve save, nao envia ROM e nao envia save completo ao servidor.
...
- A engine de batalha atual ainda e local e deterministica enquanto a engine propria definitiva e construida.
...
```bash
PYTHONPATH=PokeCable/api python3 -m unittest discover PokeCable/api/tests
PYTHONPATH=PokeCable/backend python3 -m unittest discover PokeCable/backend/pokecable_room/tests
python3 -m compileall PokeCable/api PokeCable/backend
node --check PokeCable/frontend/app.js
```
