...
class PokecableState:
    ...
    def load_pokemon(self, save_path: Path, source: str = "party") -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"Loading {save_path} with detect_parser")
            ...
            pokemon_list = save_data.party if source == "party" else []
            ...
            logger.info(f"Loaded {len(self.pokemon_list)} Pokémon from {save_path.name}")
            ...
            self.pokemon_list = [
                {"index": 0, "name": "Pikachu", "level": 50, "nickname": "", "display": "Pikachu - Lv. 50", "object": None},
                {"index": 1, "name": "Charizard", "level": 55, "nickname": "", "display": "Charizard - Lv. 55", "object": None},
            ]
            return self.pokemon_list
        except Exception as e:
