...
class RoomManagerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_join_cross_generation_is_allowed_when_clients_support_protocol(self) -> None:
        manager = RoomManager(room_timeout_seconds=60, max_rooms=10)
        ...
        room, slot = await manager.join_room(room_name="off", password="pw", client_id="b", player_name="bob", generation=3, game="pokemon_emerald", supported_protocols=PROTOCOLS)
        self.assertEqual(slot, "B")
        self.assertEqual(room.derived_modes["A"], LEGACY_DOWNCONVERT_EXPERIMENTAL)
        self.assertEqual(room.derived_modes["B"], FORWARD_TRANSFER_TO_GEN3)
    ...
    async def test_gen1_gen2_time_capsule_mode_is_derived(self) -> None:
        manager = RoomManager(
