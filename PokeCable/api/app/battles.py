...
class BattleManager:
    def __init__(self, *, room_timeout_seconds: int = 900, max_rooms: int = 100, adapter: BattleEngineAdapter | None = None) -> None:
        ...
        self.adapter = adapter or build_battle_engine()
    ...
    async def confirm_battle(self, *, client_id: str) -> tuple[BattleRoom, PlayerSlot, bool, BattleEngineResult]:
        async with self._lock:
            room, slot = self._room_for_client_locked(client_id)
            ...
            if not room.has_both_confirmations():
                return room, slot, False, BattleEngineResult(battle_id=room.battle_id or "", logs=[], finished=False)
    ...
    async def send_action(self, *, client_id: str, action: str) -> tuple[BattleRoom, PlayerSlot, BattleEngineResult]:
        async with self._lock:
            room, slot = self._room_for_client_locked(client_id)
    ...
    async def forfeit(self, *, client_id: str) -> tuple[BattleRoom, PlayerSlot, BattleEngineResult]:
        async with self._lock:
            room, slot = self._room_for_client_locked(client_id)
            if not room.battle_id:
                raise RoomError("battle_not_started", "Batalha ainda nao iniciou.")
