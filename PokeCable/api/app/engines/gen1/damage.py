...
def _low_kick_power(weight: float) -> int:
    if weight < 10:
        return 20
    if weight < 25:
        return 40
...
def calculate_damage_gen1(
    ...
    if type_multiplier > 0:
        ...
        if move_name_lower == "lowkick":
            power = _low_kick_power(getattr(defender, "weight", 50.0))
        ...
        elif move_name_lower == "superfang":
            return max(1, math.floor(defender.current_hp / 2)), type_multiplier
        elif move_name_lower == "psywave":
            dmg = math.floor(attacker.level * (random.randint(0, 100) + 50) / 100)
            return max(1, dmg), type_multiplier

    if power <= 0 or move.damage_class == "status":
    ...
    if defense_modifier != 1.0:
        d = max(1, math.floor(d * defense_modifier))
    
    level = attacker.level
    if is_critical: level *= 2
    
    # Formula Gen 1: floor(floor(floor(2 * L / 5 + 2) * P * A / D) / 50) + 2
    v = math.floor(math.floor(math.floor(2 * level / 5 + 2) * power * a / d) / 50) + 2
