...
from pokecable_room.battle_export import canonical_team_to_battle_text, canonical_to_battle_set
...
class BattleExportTests(unittest.TestCase):
    def test_canonical_to_battle_set(self) -> None:
        battle_set = canonical_to_battle_set(dratini(), 3)
        self.assertEqual(battle_set["name"], "BLUE")
        self.assertEqual(battle_set["species"], "Dratini")
        self.assertEqual(battle_set["item"], "Dragon Scale")
        self.assertEqual(battle_set["level"], 26)
        self.assertEqual(battle_set["moves"], ["Thunder Wave", "Wrap", "Surf", "Slam"])

    def test_team_text(self) -> None:
        text = canonical_team_to_battle_text([dratini()], 3)
    ...
    def test_gen1_gets_normalized_battle_fields(self) -> None:
        ...
        pokemon.nature = None
        text = canonical_team_to_battle_text([pokemon], 1)
