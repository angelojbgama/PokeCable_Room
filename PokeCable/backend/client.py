...
from compatibility import build_compatibility_report, supported_modes_for_generation
from compatibility.matrix import (
    SAME_GENERATION,
    UNSUPPORTED,
    get_trade_mode,
)
...
from data.item_transfer_policy import resolve_item_transfer_decision
from data.species import national_to_native
from evolutions import apply_trade_evolution_to_parser, preview_trade_evolution
from logs import setup_logging
from network import PokeCableNetworkClient
...
from saves import detect_parser, find_save_files, load_config, save_config
from trade import validate_payload_for_local_save
    ...
    async with PokeCableNetworkClient(server_url) as network:
        ...
        if save_path is not None:
            ...
            try:
                ...
                if received_canonical is not None:
                    received_item_transfer = _apply_received_item_transfer(
                        parser=parser,
                        location=pokemon_location,
                ...
                update_backup_metadata(
                    metadata_path,
                    {
                        ...
                        "received_item_transfer": received_item_transfer,
                    },
                )
...
def _apply_received_item_transfer(
    *,
    parser,
    location: str,
    canonical_pokemon: CanonicalPokemon,
    ui: TerminalUI,
) -> dict | None:
    held_item = canonical_pokemon.held_item
    if held_item is None or held_item.item_id in {None, 0}:
        return None
    decision = resolve_item_transfer_decision(
        held_item.item_id,
        source_generation=held_item.source_generation or canonical_pokemon.source_generation,
        target_generation=parser.get_generation(),
        target_supports_held_items=parser.get_generation() != 1,
    )
    result = {
        ...
        "resolved_item_id": decision.resolved_item_id,
        "resolved_item_name": decision.resolved_item_name,
        "resolved_category": decision.resolved_category,
        "disposition": decision.disposition,
        "fallback_disposition": decision.fallback_disposition,
        "preferred_pocket_name": decision.preferred_pocket_name,
        "fallback_pocket_name": decision.fallback_pocket_name,
        "reason": decision.reason,
        "stored_in": None,
    }
    if decision.disposition == "keep_held":
        return result
    if parser.get_generation() != 1:
        parser.clear_held_item(location)
    if decision.disposition == "remove" or decision.resolved_item_id in {None, 0}:
        ui.print(f"Item recebido removido: {held_item.name or held_item.item_id}.")
        return result
    if decision.disposition == "move_to_bag":
        if parser.has_bag_space(decision.resolved_item_id, 1):
            stored = parser.store_item_in_bag(decision.resolved_item_id, 1)
            result["stored_in"] = {"storage": stored.storage, "pocket_name": stored.pocket_name, "quantity_added": stored.quantity_added}
            ui.print(f"Item recebido movido para a mochila: {stored.item_name}.")
            return result
        if decision.fallback_disposition == "move_to_pc" and parser.has_pc_space(decision.resolved_item_id, 1):
            stored = parser.store_item_in_pc(decision.resolved_item_id, 1)
            result["disposition"] = "move_to_pc"
            result["stored_in"] = {"storage": stored.storage, "pocket_name": stored.pocket_name, "quantity_added": stored.quantity_added}
            ui.print(f"Mochila cheia. Item recebido movido para o PC: {stored.item_name}.")
            return result
        raise RuntimeError(
            f"Nao ha espaco para armazenar {decision.resolved_item_name or decision.resolved_item_id} "
            "na mochila nem no PC do save de destino."
        )
    if decision.disposition == "move_to_pc":
        if parser.has_pc_space(decision.resolved_item_id, 1):
            stored = parser.store_item_in_pc(decision.resolved_item_id, 1)
            result["stored_in"] = {"storage": stored.storage, "pocket_name": stored.pocket_name, "quantity_added": stored.quantity_added}
            ui.print(f"Item recebido movido para o PC: {stored.item_name}.")
            return result
        raise RuntimeError(f"Nao ha espaco no PC para armazenar {decision.resolved_item_name or decision.resolved_item_id}.")
    return result
