...
from canonical import CanonicalPokemon, CanonicalSpecies
from parsers.gen2 import (
    BOX_NICK_OFFSET,
    BOX_OT_OFFSET,
...
def synthetic_save() -> bytes:
    ...
    data[CRYSTAL_LAYOUT.current_box_offset] = 0
    data[CRYSTAL_LAYOUT.current_box_data_offset] = 1
    data[CRYSTAL_LAYOUT.current_box_data_offset + 1] = 201
    data[CRYSTAL_LAYOUT.current_box_data_offset + 2] = 0xFF
    box_mon_start = CRYSTAL_LAYOUT.current_box_data_offset + 0x16
    data[box_mon_start] = 201
    data[box_mon_start + 0x01] = 0x8F
    data[box_mon_start + 0x06 : box_mon_start + 0x08] = (22222).to_bytes(2, "big")
    data[box_mon_start + 0x1F] = 33
    data[CRYSTAL_LAYOUT.current_box_data_offset + BOX_OT_OFFSET : CRYSTAL_LAYOUT.current_box_data_offset + BOX_OT_OFFSET + NAME_SIZE] = encode_name("CHRIS")
    data[CRYSTAL_LAYOUT.current_box_data_offset + BOX_NICK_OFFSET : CRYSTAL_LAYOUT.current_box_data_offset + BOX_NICK_OFFSET + NAME_SIZE] = encode_name("UNOWN")
    write_checksum(data)
    return bytes(data)
...
class Gen2ParserTests(unittest.TestCase):
    def test_lists_and_replaces_party_pokemon(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertEqual(party[0].gender, "♀")
            self.assertEqual(party[1].gender, "♀")
    ...
    def test_list_boxes_reads_current_box_entries(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertEqual(boxes[0].species_name, "Unown")
            self.assertEqual(boxes[0].national_dex_id, 201)
            self.assertEqual(boxes[0].held_item_name, "Metal Coat")
            self.assertEqual(boxes[0].level, 33)
            self.assertIsNone(boxes[0].gender)
            self.assertEqual(boxes[0].unown_form, "A")
            self.assertIn("Unown (A)", boxes[0].display_summary)
