...
- O `/health` expõe o status do adapter Showdown: `local_fallback`, `process_worker`, `http_bridge` ou `showdown_unavailable`.
...
- `SHOWDOWN_REQUIRED=true` faz o startup falhar se o worker/bridge real nao responder a `ping`.
- Sem worker/bridge real, o servidor cai para `local_fallback`, suficiente para protocolo e testes, mas nao para batalha completa.
