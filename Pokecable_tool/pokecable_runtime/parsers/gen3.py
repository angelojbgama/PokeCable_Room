...
PC_BOX_COUNT = 14
PC_BOX_CAPACITY = 30
PC_BUFFER_CURRENT_BOX_OFFSET = 0x0000
...
class Gen3Parser:
    ...
    def list_party(self) -> list[PokemonSummary]:
        ...
        for index in range(count):
            ...
            gender = None if details["is_egg"] or national_dex_id is None else gender_from_gen3_personality(national_dex_id, int.from_bytes(raw[0:4], "little"))
            unown_form = None if details["is_egg"] or national_dex_id != 201 else gen3_unown_form(int(details["species_id"]), int.from_bytes(raw[0:4], "little"))
            party.append(
                PokemonSummary(
    ...
    def list_inventory(self) -> list[InventoryEntry]:
        ...
        for pocket_name in ("pc_items", "items", "key_items", "balls", "tm_hm", "berries"):
            pocket = layout.pocket(pocket_name)
            for item_id, quantity in self._read_item_slots(pocket_name).items():
                entries.append(
                    InventoryEntry(
                        item_id=item_id,
                        item_name=item_name(item_id, 3) or f"Item #{item_id}",
                        quantity=quantity,
                        generation=3,
                        storage=pocket.storage,
                        pocket_name=pocket_name,
                        category=item_category(item_id, 3),
                    )
                )
        return entries
    ...
    def export_pokemon(self, location: str) -> PokemonPayload:
        ...
        canonical_traits = canonical.metadata if isinstance(canonical.metadata, dict) else {}
        return PokemonPayload(
            ...
            metadata={
                ...
                "layout": self._require_layout().name,
                "gender": summary.gender,
                "unown_form": summary.unown_form,
                "personality": canonical_traits.get("personality"),
                "wurmple_branch": canonical_traits.get("wurmple_branch"),
                "spinda_pattern": canonical_traits.get("spinda_pattern"),
            },
    ...
    def export_canonical(self, location: str) -> CanonicalPokemon:
        ...
        metadata = {
            ...
            "is_egg": is_egg,
            "is_shiny": _gen3_is_shiny(personality, summary.trainer_id),
            "gender": summary.gender,
            "unown_form": summary.unown_form,
        ...
        metadata.update(gen3_species_pid_traits(national_id, personality))
    ...
    def build_party_mon_from_canonical(self, canonical_pokemon: CanonicalPokemon) -> bytes:
        ...
        personality = self._canonical_personality(canonical_pokemon)
        ...
        EV_CAP = 252
        source_generation = int(canonical_pokemon.source_generation or 3)
        def _to_gen3_ev(stat_exp):
            return min(EV_CAP, int(stat_exp or 0) // 256) if source_generation in {1, 2} else min(EV_CAP, int(stat_exp or 0))
        ...
        iv_data = 0
        def _to_gen3_iv(value):
            numeric = max(0, min(15 if source_generation in {1, 2} else 31, int(value or 0)))
            return min(31, numeric * 2) if source_generation in {1, 2} else numeric

        if canonical_pokemon.ivs:
            hp_iv = _to_gen3_iv(canonical_pokemon.ivs.hp)
            atk_iv = _to_gen3_iv(canonical_pokemon.ivs.attack)
            def_iv = _to_gen3_iv(canonical_pokemon.ivs.defense)
            spd_iv = _to_gen3_iv(canonical_pokemon.ivs.speed)
            spatk_iv = _to_gen3_iv(canonical_pokemon.ivs.special_attack or canonical_pokemon.ivs.special or 0)
            spdef_iv = _to_gen3_iv(canonical_pokemon.ivs.special_defense or canonical_pokemon.ivs.special or 0)
    ...
    def _parse_pokemon(self, raw: bytes) -> dict[str, int | bool]:
        ...
        return self._parse_boxed_pokemon(raw[:BOX_MON_SIZE])

    def _parse_boxed_pokemon(self, box: bytes) -> dict[str, int | bool]:
        if len(box) != BOX_MON_SIZE:
            raise ValueError("Struct Box Pokemon Gen 3 precisa ter 80 bytes.")
    ...
    def _canonical_personality(self, canonical_pokemon: CanonicalPokemon) -> int:
        ...
        for field in ("personality", "pid", "forced_pid"):
            ...
            if candidate is not None:
                return candidate
        return self._deterministic_personality(canonical_pokemon)

    def _security_key(self) -> int:
        return int.from_bytes(self._require_data()[self._section_offset(0) + 0xAC : self._section_offset(0) + 0xB0], "little")

    def _pc_buffer(self) -> bytes:
        data = self._require_data()
        chunks: list[bytes] = []
        for section_id in range(5, 13):
            offset = self._section_offset(section_id)
            chunks.append(bytes(data[offset : offset + SECTOR_DATA_SIZE]))
        offset_13 = self._section_offset(13)
        chunks.append(bytes(data[offset_13 : offset_13 + 2000]))
        return b"".join(chunks)

    def _parse_box_summary(self, raw: bytes, box_index: int, slot_index: int) -> PokemonSummary:
        details = self._parse_boxed_pokemon(raw)
        if int(details["species_id"]) <= 0:
        ...
        ot_name = self._decode_text(raw[0x14 : 0x14 + OT_NAME_SIZE])
        national_dex_id = None if details["is_egg"] else native_to_national(3, int(details["species_id"]))
        level = 1 if details["is_egg"] or national_dex_id is None else level_from_species_experience(national_dex_id, int(details["experience"]))
        held_item_id = int(details["held_item_id"]) if details.get("held_item_id") else None
        gender = None if details["is_egg"] or national_dex_id is None else gender_from_gen3_personality(national_dex_id, int.from_bytes(raw[0:4], "little"))
        unown_form = None if details["is_egg"] or national_dex_id != 201 else gen3_unown_form(int(details["species_id"]), int.from_bytes(raw[0:4], "little"))
        return PokemonSummary(
    ...
    def _store_item_in_pocket(self, pocket_name: str, item_id: int, quantity: int) -> InventoryStoreResult:
        ...
        if new_quantity > GEN3_MAX_STACK:
            raise ValueError("Stack de itens excedeu 999 unidades.")
